...
Bebopt.prototype.__hasChildParent = function(opt, noneCb, childCb, parentCb, bothCb) {
  ...
  if (opt.child === null) {
    if (opt.parent === null) {
      return noneCb(callbacks);
    } else {
      return parentCb(callbacks);
    }
  } else {
    if (opt.parent === null) {
      return childCb(callbacks);
    } else {
      return bothCb(callbacks);
    }
  }
};
...
// when hasChild, do CB1; if hasParent then CB2; if has both,
// then Cb1 and Cb2
Bebopt.prototype._hasBothChildParent = function(opt, noneCb, childCb, parentCb) {
  return this.__hasChildParent(opt,
    noneCb,
    childCb,
    parentCb,
    function() { childCb(); parentCb(); });
};
...
Bebopt.prototype._setOptionArg = function(opt, listName) {
  ...
  self.__hasChildParent(opt,
    function() { /* none */ },
    function() { self._options[option.child.index].arg = opt.optarg; }, // child
    function() { self._options[option.parent.index].arg = opt.optarg; }, // parent
    function(context) { context.childCb(); context.parentCb(); }); // both
};
...
Bebopt.prototype._buildCleanContext = function() {
  var self = this
    , context = clone(self)
    ;
